...
export async function GET(_req:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 const {rows}=await query(`SELECT id,subject,source_name,attachment_name,total_recipients,sent_count,failed_count,status,created_at,completed_at FROM campaigns WHERE id=$1 AND user_id=$2 LIMIT 1`,[id,u.id]);
