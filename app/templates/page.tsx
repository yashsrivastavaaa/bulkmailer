...
export default function Templates(){
 ...
 const [templates,setTemplates]=useState<any[]>([]); const [loading,setLoading]=useState(false); const [saving,setSaving]=useState(false); const [form,setForm]=useState({name:'',subject:'',body:''}); const [error,setError]=useState('');
 async function boot(){
   try{const sr=await fetch('/api/auth/status',{cache:'no-store'});const sd=await sr.json();if(!sd.connected){setAuth('guest');return;}setAuth('ready');setLoading(true);
