...
export default function CampaignDetail({params}:{params:Promise<{id:string}>}){
 ...
 return <div>
  <div className="floating-page-head">
   <div><Link href="/campaigns" className="back-link">â† Campaigns</Link><div className="eyebrow">CAMPAIGN DETAILS</div><h1>{campaign.subject}</h1><p>{campaign.source_name||'Manual list'}{campaign.attachment_name?` Â· ${campaign.attachment_name}`:''} Â· Created {new Date(campaign.created_at).toLocaleString()}</p></div>
  ...
  <section className="campaign-overview">
   ...
   <div className="campaign-overview-card"><span>{campaign.track_opens?'Opened':'Pending'}</span><strong>{campaign.track_opens?recipients.filter(r=>r.opened_at).length.toLocaleString():(counts.pending+counts.sending).toLocaleString()}</strong><small>{campaign.track_opens?'Estimated email opens':'Awaiting delivery'}</small></div>
  </section>
  <section className="float-panel campaigns-panel">
   <div className="campaigns-toolbar"><div><h2>Recipient activity</h2><p>Live delivery activity for every recipient. Status refreshes automatically while sending.</p></div><div className="filter-row"><label className="search-wrap">âŒ•<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search email or nameâ€¦"/></label><SelectMenu className="filter-select" value={statusFilter} onChange={setStatusFilter} options={[{value:"all",label:"All statuses"},{value:"sent",label:"Sent"},{value:"sending",label:"Sending"},{value:"pending",label:"Pending"},{value:"failed",label:"Failed"}]} aria-label="Recipient status" /></div></div>
