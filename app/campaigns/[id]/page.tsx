...
export default function CampaignDetail({params}:{params:Promise<{id:string}>}){
 const [campaign,setCampaign]=useState<Campaign|null>(null);const [recipients,setRecipients]=useState<Recipient[]>([]);const [loading,setLoading]=useState(true);const [error,setError]=useState('');const [search,setSearch]=useState('');
 ...
 const filtered=useMemo(()=>recipients.filter(r=>!search.trim()||r.email.toLowerCase().includes(search.toLowerCase())||(r.name||'').toLowerCase().includes(search.toLowerCase())),[recipients,search]);
 ...
 if(loading)return <div className="route-loading"><div className="loading-orb"/><div className="loading-copy"><strong>Loading campaign</strong><span>Fetching recipient delivery status…</span></div></div>;
