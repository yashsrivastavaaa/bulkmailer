...
type Recipient={id:string;email:string;name?:string;selected:boolean;status:string;error?:string;sent_at?:string;provider_message_id?:string;opened_at?:string;open_count?:number};
type Campaign={id:string;subject:string;source_name?:string;attachment_name?:string;total_recipients:number;sent_count:number;failed_count:number;status:string;created_at:string;completed_at?:string;track_opens?:boolean};
...
export default function CampaignDetail({params}:{params:Promise<{id:string}>}){
 ...
 return <div>
  <div className="floating-page-head">
   <div><Link href="/campaigns" className="back-link">&larr; Campaigns</Link><div className="eyebrow">CAMPAIGN DETAILS</div><h1>{campaign.subject}</h1><p>{campaign.source_name||'Manual list'}{campaign.attachment_name?` · ${campaign.attachment_name}`:''} · Created {new Date(campaign.created_at).toLocaleString()}</p></div>
  ...
  <section className="campaign-overview">
   ...
   <div className="campaign-overview-card"><span>{campaign.track_opens?'Opened':'Pending'}</span><strong>{campaign.track_opens?recipients.filter(r=>r.opened_at).length.toLocaleString():(counts.pending+counts.sending).toLocaleString()}</strong><small>{campaign.track_opens?<span className="tracking-estimate">Estimated opens <span className="tracking-tooltip"><Info size={13} aria-label="Open tracking is estimated: image blocking, privacy protection, and preloading can affect the count."/><span>Open tracking is estimated. Image blocking, privacy protection and email-client preloading can affect this count.</span></span></span>:'Awaiting delivery'}</small></div>
  </section>
  <section className="float-panel campaigns-panel">
   <div className="campaigns-toolbar"><div><h2>Recipient activity</h2><p>Live delivery activity for every recipient. Status refreshes automatically while sending.</p></div><div className="filter-row"><label className="search-wrap"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search email or name..."/></label><SelectMenu className="filter-select" value={statusFilter} onChange={setStatusFilter} options={[{value:"all",label:"All statuses"},{value:"sent",label:"Sent"},{value:"sending",label:"Sending"},{value:"pending",label:"Pending"},{value:"failed",label:"Failed"}]} aria-label="Recipient status" /></div></div>
   {!filtered.length?<div className="empty-state campaign-empty"><h2>No recipients found</h2><p>Try another search.</p></div>:<div className="campaign-table recipient-status-table"><div className="campaign-row campaign-header"><span>Recipient</span><span>Status</span><span>Sent at</span><span>Message ID</span><span>Details</span></div>{filtered.map(r=><div className="campaign-row" key={r.id}><span className="campaign-main"><b>{r.name||r.email.split('@')[0]}</b><small>{r.email}</small></span><span><span className={statusClass(r.status)}><i/>{label(r.status)}</span></span><span><b>{r.sent_at?new Date(r.sent_at).toLocaleDateString():'â€”'}</b><small>{r.sent_at?new Date(r.sent_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):r.status==='pending'?'Waiting':'â€”'}</small></span><span><small className="mono-text">{r.provider_message_id||'â€”'}</small></span><span><small className={r.error?'danger-text':''}>{r.error|| (r.status==='sent'?(campaign.track_opens?(r.opened_at?`Opened ${r.open_count||1} time${r.open_count===1?'':'s'}`:'Not opened yet'):'Delivered successfully'):r.status==='sending'?'Sending now':'Waiting to send')}</small></span></div>)}</div>}
  </section>
 </div>
}
