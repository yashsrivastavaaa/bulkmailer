...
export async function getUserPlan(userId:string){
  const { rows } = await query<any>(`
    SELECT
      p.slug AS key,
      p.name,
      p.price_monthly_cents AS price_cents,
      p.monthly_send_limit,
      p.max_recipients_per_campaign AS campaign_recipient_limit,
      p.max_attachment_mb AS attachment_limit_mb,
      p.max_custom_columns,
      p.features,
  ...
  // Existing databases may contain a subscription row whose plan_id is NULL.
  // Always select the plan id here and repair/update the existing row instead
  // of using DO NOTHING, otherwise auth/status returns HTTP 500 after OAuth.
  const free = await query<any>(`
    SELECT id, slug AS key, name, price_monthly_cents AS price_cents,
           monthly_send_limit,
           max_recipients_per_campaign AS campaign_recipient_limit,
           max_attachment_mb AS attachment_limit_mb,
           max_custom_columns, features
    FROM plans
    WHERE slug='free'
    LIMIT 1
  `);

  if (!free.rows[0]) return rows[0];
  ...
  return { ...free.rows[0], status:'active', role:rows[0].role };
}
